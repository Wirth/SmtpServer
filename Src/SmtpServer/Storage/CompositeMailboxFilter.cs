...
namespace SmtpServer.Storage
{
    internal sealed class CompositeMailboxFilter : IMailboxFilter
    {
        ...
        public async Task<MailboxFilterResult> CanAcceptFromAsync(ISessionContext context, IMailbox @from, int size = 0)
        {
            ...
            var results = await Task.WhenAll(_filters.Select(f => f.CanAcceptFromAsync(context, @from, size))).ConfigureAwait(false);
        ...
        public async Task<MailboxFilterResult> CanDeliverToAsync(ISessionContext context, IMailbox to, IMailbox @from)
        {
            ...
            var results = await Task.WhenAll(_filters.Select(f => f.CanDeliverToAsync(context, to, @from))).ConfigureAwait(false);
